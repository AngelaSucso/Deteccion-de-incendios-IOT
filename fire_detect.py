...
def main(image_path, resize_width, resize_height):
    ...
    # Si NO hay detecciones → retorna False
    if result.boxes is None or len(result.boxes) == 0:
        return False

    # Hay detección → generar imagen anotada
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resize_height', type=int, default=600)
    args = parser.parse_args()

    result = main(args.image_path, args.resize_width, args.resize_height)

    if result is False:
        print("No se detecto fuego")
    else:
        detected, image_path = result
